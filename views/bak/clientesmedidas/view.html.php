...
class InvoicesViewClientesmedidas extends JViewLegacy
{
    ...
    protected function getCamposSelect($cliente)
    {
        if (count($this->camposdedatos['campos']) > 0) {
            ...
            foreach (($this->camposdedatos['campos']) as $campo) {
                if (trim($campo['forma']) == 'select' && trim($campo['sql']) != '') {
                    $helper = new HelperComponent;
                    $select = $campo['id'];
                    $sqlstring= str_replace("#ID#",$cliente,$campo['sql']);
                    $camposselect = $helper->getSelectData($sqlstring);
                    if (!empty($camposselect)) {
                        if (count($camposselect[0]) > 2) {
                            ...
                            $sqlstring= str_replace("#ID#",$cliente,$campo['sql']);
                            $this->camposdedatos[$select] = $helper->getSelectData($sqlstring);
                        }
                    }
                }
            }
